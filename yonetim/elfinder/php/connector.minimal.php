<?php session_start(); if(isset($_SESSION['tech'])){
...
$opts = array(
	...
	'roots' => array(
		...
		array(
			...
			'URL'           => 'http://'.$_SERVER['HTTP_HOST'].'/upload/', // URL to files (REQUIRED)
			'trashHash'     => 't1_Lw',                     // elFinder's hash of trash folder
			'winHashFix'    => DIRECTORY_SEPARATOR !== '/', // to make hash same to Linux one on windows too
			'uploadMaxSize' => '5M', 						//file size
		...
		array(
			'id'            => '1',
			'driver'        => 'Trash',
			'path'          => '../../../upload/.trash/',  // path to files (REQUIRED)
			'URL'           => 'çöp bölümünden seçim yapmayınız! sadece upload klasörü altından seçim yapın.', // URL to files (REQUIRED)
			'tmbURL'        => 'http://'.$_SERVER['HTTP_HOST'].'/upload/.trash/.tmb/', // TMB URL to files (REQUIRED)
			'winHashFix'    => DIRECTORY_SEPARATOR !== '/', // to make hash same to Linux one on windows too
			'uploadDeny'    => array('all'),                // Recomend the same settings as the original volume that uses the trash
			'uploadAllow'   => array('image/x-ms-bmp', 'image/gif', 'image/jpeg', 'image/png', 'image/x-icon'), // Same as above
